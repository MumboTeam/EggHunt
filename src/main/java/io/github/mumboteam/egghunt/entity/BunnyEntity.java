...
public class BunnyEntity extends Entity implements PolymerEntity {
    ...
    public BunnyEntity(EntityType<? extends BunnyEntity> entityType, World world) {
        ...
        this.background.setText(Text.literal("1").setStyle(Style.EMPTY.withFont(new StyleSpriteSource.Font(Identifier.of(EggHunt.ID, "background")))));
    ...
    public void onBeforeSpawnPacket(ServerPlayerEntity player, Consumer<Packet<?>> packetConsumer) {
        ...
        GameProfile gameprofile = new GameProfile(this.getUuid(), "", properties);
    ...
    public void updateLeaderboard(EggHuntState state) {
        ...
        Style style = Style.EMPTY.withFont(new StyleSpriteSource.Font(Identifier.of(EggHunt.ID, "mono")));
        AtomicInteger count = new AtomicInteger(0);

        state.playerData.entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.comparingInt(PlayerData::getEggCount).reversed())).limit(10).forEach(entry -> {
            ...
            MutableText line = Text.literal(String.format(" %2d. %-16s - %3d \n", count.incrementAndGet(), name, entry.getValue().getEggCount()));
            if (count.get() == 1) {
        ...
        if (eggs.get() == 0) {
            ...
            state.playerData.compute(player.getUuid(), (uuid, data) -> {
                if (data == null) {
                    ...
                    pData.increaseEggCount(eggs.get());
                    return pData;
                } else {
                    data.increaseEggCount(eggs.get());
                    return data;
                }
            });
